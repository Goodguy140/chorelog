...
function resetVapidConfiguration() {
  vapidConfigured = false;
}

function getPrivateVapidKey() {
  const k = process.env.CHORELOG_VAPID_PRIVATE_KEY;
  return k && String(k).trim() ? String(k).trim() : '';
}

function getVapidSubject() {
  return String(process.env.CHORELOG_VAPID_SUBJECT || 'mailto:noreply@localhost').trim();
}

/**
 * Updates process.env and clears the web-push cache so the next send re-reads keys.
 * @returns {boolean} whether web-push accepted the key pair
 */
function setRuntimeVapidKeys(publicKey, privateKey, subject) {
  process.env.CHORELOG_VAPID_PUBLIC_KEY = String(publicKey || '').trim();
  process.env.CHORELOG_VAPID_PRIVATE_KEY = String(privateKey || '').trim();
  process.env.CHORELOG_VAPID_SUBJECT = String(subject || '').trim() || 'mailto:noreply@localhost';
  vapidConfigured = false;
