...
function normalizeHhMm(raw, fallback) {
  const v = typeof raw === 'string' ? raw.trim() : '';
  return /^\d{2}:\d{2}$/.test(v) ? v : fallback;
}

function normalizePushPreferencePrefs(raw) {
  const src = raw && typeof raw === 'object' ? raw : {};
  const quietHoursEnabled =
    src.quietHoursEnabled === true ? true : src.quietHoursEnabled === false ? false : null;
  return {
    enabled: src.enabled !== false,
    overdue: src.overdue !== false,
    dueToday: src.dueToday !== false,
    quietHoursEnabled,
    quietHoursStart: normalizeHhMm(src.quietHoursStart, '22:00'),
    quietHoursEnd: normalizeHhMm(src.quietHoursEnd, '08:00'),
  };
}
...
function normalizePushSubscription(row) {
  ...
  const member = typeof row.member === 'string' ? row.member.trim().slice(0, 120) : '';
  ...
  return {
    ...
    createdAt,
    member,
    prefs: normalizePushPreferencePrefs(row.prefs),
  };
}
