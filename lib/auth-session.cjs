...
function buildRequireApiAuth({
  ensureRegistryLoaded,
  getRegistry,
  verifyAuthCookieFn = verifyAuthCookie,
  verifyApiTokenFn = null,
}) {
  return async function requireApiAuth(req, res, next) {
    ...
    if (verifyApiTokenFn) {
      ...
      const bearer = m && m[1] ? m[1].trim() : '';
      if (bearer) {
        const tokenPayload = await verifyApiTokenFn(bearer);
        if (!tokenPayload) {
          return res.status(401).json({ error: 'Unauthorized' });
        }
        req.authPayload = tokenPayload;
        req.householdId = tokenPayload.household;
        return next();
      }
    }
