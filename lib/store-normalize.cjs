...
function normalizeEntry(row) {
  ...
  if (row.attachment && typeof row.attachment === 'object') {
    const mime = typeof row.attachment.mime === 'string' ? row.attachment.mime.trim().toLowerCase().split(';')[0] : '';
    const allowed = new Set(['image/jpeg', 'image/png', 'image/webp', 'image/gif']);
    if (mime && allowed.has(mime)) {
      let bytes = Number(row.attachment.bytes);
      if (!Number.isFinite(bytes) || bytes < 0) bytes = 0;
      out.attachment = { mime, bytes };
    }
  }
...
function normalizeStore(raw) {
  ...
  const apiTokens = Array.isArray(safeRaw.apiTokens)
    ? safeRaw.apiTokens
      ...
      .map((row) => {
        ...
        const tokenHash = typeof row.tokenHash === 'string' ? row.tokenHash.trim() : '';
        if (!id || !label || !tokenHash) return null;
        const scope = row.scope === 'ro' ? 'ro' : 'rw';
        const ownerType = row.ownerType === 'personal' ? 'personal' : 'household';
        const ownerUser =
          ownerType === 'personal' && typeof row.ownerUser === 'string'
            ? row.ownerUser.trim().slice(0, 120)
            : '';
        const createdAt =
        ...
        const lastUsedAt =
          typeof row.lastUsedAt === 'string' && row.lastUsedAt.includes('T') ? row.lastUsedAt : null;
        const revokedAt =
          typeof row.revokedAt === 'string' && row.revokedAt.includes('T') ? row.revokedAt : null;
        return {
          id,
          label,
          tokenHash,
